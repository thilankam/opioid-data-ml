...
Next, clone this repository by opening a terminal and typing the following commands:
    ...
    $ git clone https://github.com/thilankam/opioid-data-ml.git
    $ cd opioid-data-ml
...
When using Anaconda, you can optionally create an isolated Python environment dedicated to this project. This is recommended as it makes it possible to have a different environment for each project (e.g. one for this project), with potentially different libraries and library versions:
    ...
    $ conda create -n opioid-data-ml python=3.5 anaconda
    $ source activate opioid-data-ml

This creates a fresh Python 3.5 environment called `opioid-data-ml` (you can change the name if you want to), and it activates it. This environment contains all the scientific libraries that come with Anaconda. This includes all the libraries we will need (NumPy, Matplotlib, Pandas, Jupyter and a few others), except for TensorFlow, so let's install it:

    $ conda install -n opioid-data-ml -c conda-forge tensorflow=1.0.0

This installs TensorFlow 1.0.0 in the `opioid-data-ml` environment (fetching it from the `conda-forge` repository). If you chose not to create an `opioid-data-ml` environment, then just remove the `-n opioid-data-ml` option.

Next, you can optionally install Jupyter extensions. These are useful to have nice tables of contents in the notebooks, but they are not required.

    $ conda install -n opioid-data-ml -c conda-forge jupyter_contrib_nbextensions
